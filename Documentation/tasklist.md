# Tasklist
 This indicates a task is yet to be completed. ~This indicates a task was completed.~
## 1. Outline & Code Core Game Features
* Wall Collision
* Gravity
* Left Movement (Mapped onto < key)
* Right Movement (Mapped onto > key)
* Crouch (Mapped onto v key)
* Jump (Mapped onto Space Bar; single tap)
* Double Jump (Mapped onto Space Bar; double tap)
* Set up mini-level to test mechanics
* Ice Wall Climb (Player climbs up if they use ^ key; player slides down if they use v key; player jumps off wall if they use Space Bar)
* Grappling Hook (Mapped onto Z key; pulls Player to a location when launched)

## 2. Fine-Tune Core Game Mechanics & Additional Mechanics
* Dark Mask on level; only clear around Player and fixed light sources
* Double Jump only allowed after touching a certain item
* Respawn Spots
* Enemy AI
* Assign stock sounds to Movement (Walk Left/Right, Crouch, Jump, Ice Wall Attach + Detach, Grappling Hook Launch + Attach, Hurt)

## 3. Outline Levels, Win Conditions, & Lose Conditions
* Tutorial Level
* Win Condition
* Lose Condition

## 4. Graphics & Audio
* Player Character Animations
* Menu Music
* Victory Music
